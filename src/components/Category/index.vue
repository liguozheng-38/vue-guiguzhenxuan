    ...
    <el-form :inline="true" class="category-form">
      <el-form-item label="一级分类">
        ...
        <el-select :disabled="scene == 0 ? false : true" v-model="categoryStore.c1Id" @change="handler">
          ...
          <el-option v-for="c1 in categoryStore.c1Arr" :key="c1.id" :label="c1.name" :value="String(c1.id)"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="二级分类">
        <el-select :disabled="scene == 0 ? false : true" v-model="categoryStore.c2Id" @change="handler1">
          <el-option v-for="c2 in categoryStore.c2Arr" :key="c2.id" :label="c2.name" :value="String(c2.id)"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="三级分类">
        <el-select :disabled="scene == 0 ? false : true" v-model="categoryStore.c3Id" @change="handler2">
          <el-option v-for="c3 in categoryStore.c3Arr" :key="c3.id" :label="c3.name" :value="String(c3.id)"></el-option>
        </el-select>
      </el-form-item>
    </el-form>
  </el-card>
...
<script setup lang="ts">
  ...
  const loadC2 = async () => {
    try {
      ...
      if (categoryStore.c2Arr.length === 0 && categoryStore.c1Id !== null && categoryStore.c1Id !== undefined) {
  ...
  const loadC3 = async () => {
    try {
      ...
      if (categoryStore.c3Arr.length === 0 && categoryStore.c2Id !== null && categoryStore.c2Id !== undefined) {

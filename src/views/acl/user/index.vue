    ...
    <el-table @selection-change="selectChange" style="margin: 10px 0px" border :data="userArr">
      ...
      <el-table-column label="操作" width="300px" align="center">
        <template #="{ row }">
          ...
          <el-popconfirm :title="`你确定要删除${row.username}?`" width="260px" @confirm="deleteUser(row.id ?? row.ID)">
            <template #reference>
...
<script setup lang="ts">
  ...
  const setRole = async (row: User) => {
    Object.assign(userParams, row)
    let result: AllRoleResponseData = await reqAllRole(row.id ?? (row.ID as number))
    if (result.code == 200) {
      allRole.value = result.data.allRolesList
      userRoleIds.value = result.data.assignRoles.map((r) => r.id ?? (r.ID as number))
      drawer1.value = true
    }
  }
  ...
  const handleCheckAllChange = (val: any) => {
    userRoleIds.value = val ? allRole.value.map((r) => r.id ?? (r.ID as number)) : []
  ...
  const handleCheckedCitiesChange = (value: any[]) => {
    ...
    isIndeterminate.value = numValues.length > 0 && numValues.length !== allRole.value.length
  }
  ...
  const deleteSelectUser = async () => {
    ...
    if (result.code == 200) {
      ...
      getHasUser(userArr.value.length > selectIdArr.value.length ? pageNo.value : Math.max(1, pageNo.value - 1))
      selectIdArr.value = []
    }
  }

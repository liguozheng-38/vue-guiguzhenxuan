    ...
    <el-form-item label="SPU销售属性">
      ...
      <el-table border style="margin: 10px 0px" :data="saleAttr">
        ...
        <el-table-column label="销售属性值">
          ...
          <template #="{ row }">
            <el-tag style="margin: 0px 5px" @close="row.spuSaleAttrValueList.splice(index, 1)" v-for="(item, index) in row.spuSaleAttrValueList" :key="`${row.id}-${index}`" class="mx-1" closable>

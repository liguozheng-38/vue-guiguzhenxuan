  ...
  <div>
    <el-card class="box-card">
      ...
      <!-- 分页器组件
            ...
            -->
      <el-pagination @size-change="sizeChange" @current-change="getHasTrademark" :pager-count="9" v-model:current-page="pageNo" v-model:page-size="limit" :page-sizes="[3, 5, 7, 9]" :background="true" layout="prev, pager, next, jumper,->,sizes,total" :total="total" />
    </el-card>
...
<script setup lang="ts">
  ...
  const getHasTrademark = async (pager = 1) => {
    //当前页码
    pageNo.value = pager
    let result: TradeMarkPageResponseData = await reqHasTrademark(pageNo.value, limit.value)
  ...
  const confirm = async () => {
    try {
      ...
      // console.log('表单校验通过，准备提交数据:', trademarkParams)

      //准备提交的数据
      const submitData = trademarkParams.id
        ? { ...trademarkParams }
        : {
            tmName: trademarkParams.tmName,
            logoUrl:
              trademarkParams.logoUrl ||
              'data:image/svg+xml;base64,',
          }

      let result: any = await reqAddOrUpdateTrademark(submitData)
      ...
      if (result && result.ok === true) {
        ...
        ElMessage({
          type: 'error',
          message: result?.message || (trademarkParams.id ? '修改品牌失败' : '添加品牌失败'),
        })
      }
    } catch (error: any) {

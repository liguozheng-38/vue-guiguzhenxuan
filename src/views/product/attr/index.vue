        ...
        <el-table border style="margin: 10px 0px" :data="attrArr">
          ...
          <el-table-column label="操作" width="120px">
            ...
            <template #="{ row }">
              ...
              <el-popconfirm :title="`你确定删除${row.attrName}?`" width="200px" @confirm="deleteAttr(row.id ?? row.ID)">
                <template #reference>
      ...
      <div v-show="scene == 1">
        ...
        <el-table border style="margin: 10px 0px" :data="attrParams.attrValueList">
          ...
          <el-table-column label="属性值名称">
            <!-- row:即为当前属性值对象 -->
            <template #="{ row, $index }">
              <el-input :ref="(vc: any) => (inputArr[$index] = vc)" v-if="row.flag" @blur="toLook(row, $index)" size="small" placeholder="请你输入属性值名称" v-model="row.valueName"></el-input>
...
<script setup lang="ts">
  ...
  const getAttr = async () => {
    ...
    if (c1Id == null || c2Id == null || c3Id == null) return
    try {
      let result: AttrResponseData = await reqAttr(c1Id, c2Id, c3Id)
      if (result.code == 200) {
        // 数据映射：后端返回name和children，前端需要attrName和attrValueList
        const mappedData = (result.data || []).map((item: Attr) => {
          return {
            ...item,
            attrName: item.name || item.attrName || '',
            attrValueList: (item.children || item.attrValueList || []).map((child: any) => {
              if (typeof child === 'string') {
                return { valueName: child, flag: false }
              }
              return { ...child, valueName: child.valueName || child.name || '', flag: false }
            }),
          }
        })
